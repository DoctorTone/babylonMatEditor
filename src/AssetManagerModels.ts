...
export const AssetManagerFallback: React.FC<AssetManagerFallbackType> = ({
  barColor,
  textColor,
  totalControls,
}) => {
  ...
  useEffect(() => {
    if (controlRefs.current && context?.lastProgress) {
      ...
      controlRefs.current.textRef.text = `loaded ${
        eventData.totalCount - eventData.remainingCount
      ...
      controlRefs.current.progressText.text = `${(
        percentComplete * 100
      ).toFixed(0)}%`;
    }
...
const AssetManagerModels: React.FC<AssetManagerModels> = ({
  boomboxPosition,
  avocadoPosition,
}) => {
